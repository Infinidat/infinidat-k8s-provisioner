...
func (p *FCProvisioner) UpdateExport(pvList []*v1.PersistentVolume, nodeList []*v1.Node) error {
...
func (p *FCProvisioner) getHostList(k8sNodeList []*v1.Node) (list []string, err error) {
	...
	for _, hostFromInfiniBox := range arrayOfHosts {
		...
		for _, node := range k8sNodeList {
			if node.Name == hostCastedInMap["name"] {
