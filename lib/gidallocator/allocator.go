...
func (a *Allocator) AllocateNext(options controller.VolumeOptions) (int, error) {
	class := controller.GetPersistentVolumeClaimClass(options.PVC)
...
func (a *Allocator) Release(volume *v1.PersistentVolume) error {
	class, err := a.client.Storage().StorageClasses().Get(controller.GetPersistentVolumeClass(volume), metav1.GetOptions{})
...
func (a *Allocator) collectGids(className string, gidTable *allocator.MinMaxAllocator) error {
	...
	for _, pv := range pvList.Items {
		if controller.GetPersistentVolumeClass(&pv) != className {
			continue
		}
