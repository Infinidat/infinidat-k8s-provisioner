...
type ProvisionController struct {
	...
	/**
	map will contain provisioner name and provisioner binding
	*/
	proviserners map[string]Provisioner
	...
	configSource    cache.ListerWatcher 
	configReflector *cache.Reflector    
	...
	failedProvisionRetriedStopped, failedDeleteRetriedStopped       map[types.UID]bool
...
func NewProvisionController(
	client kubernetes.Interface,
	defaultProvisionerName string,
	/*provisioner Provisioner,*/
	supoortedProvisioners map[string]Provisioner,
	kubeVersion string,
	...
	if err != nil {
		eventRecorder = broadcaster.NewRecorder(Scheme, v1.EventSource{Component: fmt.Sprintf("%s %s", defaultProvisionerName, string(identity))})
	} else {
		eventRecorder = broadcaster.NewRecorder(Scheme, v1.EventSource{Component: fmt.Sprintf("%s %s %s", defaultProvisionerName, strings.TrimSpace(string(out)), string(identity))})
	}
	...
	controller := &ProvisionController{
		...
		provisioner:                   provisioner,*/
		proviserners:                  supoortedProvisioners,
	...
	controller.nodeSource = &cache.ListWatch{
	...
	controller.nodes, controller.nodeController = cache.NewInformer(
		controller.nodeSource,
		&v1.Node{},
		controller.resyncPeriod,
		cache.ResourceEventHandlerFuncs{
			AddFunc:    controller.nodeAdded,
			UpdateFunc: nil, //controller.nodeUpdated
	...
	controller.volumeSource = &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			pvl, err := client.CoreV1().PersistentVolumes().List(options)
			return pvl, err
		},
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
	...
	controller.configuration = cache.NewStore(cache.DeletionHandlingMetaNamespaceKeyFunc)
	controller.configSource = &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			return client.CoreV1().ConfigMaps(v1.NamespaceDefault).List(options)
		},
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
			return client.CoreV1().ConfigMaps(v1.NamespaceDefault).Watch(options)
		},
	}
	controller.configReflector = cache.NewReflector(
...
func (ctrl *ProvisionController) Run(stopCh <-chan struct{}) {
	...
	go ctrl.configReflector.Run(stopCh) 
	go ctrl.nodeController.Run(stopCh)  
...
func (ctrl *ProvisionController) nodeDeleted(obj interface{}) { 
	node, ok := obj.(*v1.Node)
	if !ok {
		return
	}
...
func (ctrl *ProvisionController) nodeUpdated(oldObj, newObj interface{}) {
	...
	glog.V(1).Info("Node addresses are updated  ", newnode.Status.Addresses)
	//call all provisioner to update export
	for pname, provisioner := range ctrl.proviserners {
		if strings.Contains(pname, "/") { //so that default provisioner will get call only once
...
func (ctrl *ProvisionController) isOnlyRecordUpdate(oldClaim, newClaim *v1.PersistentVolumeClaim) (bool, error) {
	...
	newobj, err := ctrl.removeRecord(newClaim)
	if err != nil {
		return false, err
	}
	return reflect.DeepEqual(old, newobj), nil
}
...
func (ctrl *ProvisionController) shouldProvision(claim *v1.PersistentVolumeClaim) bool {
	...
	if failureCount, exists := ctrl.failedProvisionStats[claim.UID]; exists == true {
		if failureCount >= ctrl.failedProvisionThreshold && ctrl.failedProvisionThreshold > 0 {
			if _,ex:=ctrl.failedProvisionRetriedStopped[claim.UID]; !ex{
				ctrl.failedProvisionRetriedStopped[claim.UID] = true//below threshold exceeded message should be shown once
	...
	if provisioner, found := claim.Annotations[annStorageProvisioner]; found {
		if ctrl.proviserners[provisioner] != nil {
			return true
		}
		return false
	}
...
func (ctrl *ProvisionController) shouldDelete(volume *v1.PersistentVolume) bool {
	...
	if failureCount, exists := ctrl.failedDeleteStats[volume.UID]; exists == true {
		if failureCount >= ctrl.failedDeleteThreshold && ctrl.failedDeleteThreshold > 0 {
			if _,ex:=ctrl.failedDeleteRetriedStopped[volume.UID]; !ex {
				ctrl.failedDeleteRetriedStopped[volume.UID] = true //below threshold exceeded message should be shown once
	...
	if ann := volume.Annotations[annDynamicallyProvisioned]; ctrl.proviserners[ann] == nil {
		return false
	}
...
func (ctrl *ProvisionController) provisionClaimOperation(claim *v1.PersistentVolumeClaim) error {
	...
	claimClass := GetPersistentVolumeClaimClass(claim)
	glog.V(1).Infof("provisionClaimOperation [%s] started, class: %q", claimToClaimKey(claim), claimClass)
	...
	if err == nil && volume != nil {
		...
		glog.V(1).Infof("provisionClaimOperation [%s]: volume already exists, skipping", claimToClaimKey(claim))
		return nil
	}

	// Prepare a claimRef to the claim early (to fail before a volume is
	// provisioned)
	claimRef, err := ref.GetReference(Scheme, claim)
	if err != nil {
	...
	glog.Infoln()
	volume, err = ctrl.proviserners[provisioner].Provision(options, ctrl.getConfiguration(), ctrl.getNodeList())
	if err != nil {
	...
	metav1.SetMetaDataAnnotation(&volume.ObjectMeta, annDynamicallyProvisioned, provisioner)
	...
	for i := 0; i < ctrl.createProvisionedPVRetryCount; i++ {
		glog.V(1).Infof("provisionClaimOperation [%s]: trying to save volume %s", claimToClaimKey(claim), volume.Name)
	...
	if err != nil {
		...
		for i := 0; i < ctrl.createProvisionedPVRetryCount; i++ {
			glog.Infoln()
			if err = ctrl.proviserners[provisioner].Delete(volume, ctrl.getConfiguration(), ctrl.getNodeList()); err == nil {
				// Delete succeeded
				glog.V(1).Infof("provisionClaimOperation [%s]: cleaning volume %s succeeded", claimToClaimKey(claim), volume.Name)
				break
			}
		...
		for {
			...
			case event := <-pvcCh:
				...
				case *v1.PersistentVolumeClaim:
					...
					glog.V(1).Infof("claim update received: %s %s/%s %s", event.Type, claim.Namespace, claim.Name, claim.Status.Phase)
					switch event.Type {
				...
				case *v1.Event:
					...
					glog.V(1).Infof("claim event received: %s %s/%s %s/%s %s", event.Type, claimEvent.Namespace, claimEvent.Name, claimEvent.InvolvedObject.Namespace, claimEvent.InvolvedObject.Name, claimEvent.Reason)
...
func (ctrl *ProvisionController) deleteVolumeOperation(volume *v1.PersistentVolume) error {
	glog.V(1).Infof("deleteVolumeOperation [%s] started", volume.Name)
	...
	err = ctrl.proviserners[newVolume.Annotations[annDynamicallyProvisioned]].Delete(volume, ctrl.getConfiguration(), ctrl.getNodeList())
	if err != nil {
		if ierr, ok := err.(*IgnoredError); ok {
	...
	glog.V(1).Infof("deleteVolumeOperation [%s]: success", volume.Name)
	glog.Infoln()
	// Delete the volume
...
func (ctrl *ProvisionController) scheduleOperation(operationName string, operation func() error) {
	...
	if err != nil {
		if goroutinemap.IsAlreadyExists(err) {
			glog.V(1).Infof("operation %q is already running, skipping", operationName)
		} else {
...
func (ctrl *ProvisionController) getNodeList() []*v1.Node {
	...
	for _, o := range ctrl.nodes.List() {
		nodeo, ok := o.(*v1.Node)
		if ok {
			nodelist = append(nodelist, nodeo)
		}
	}
	return nodelist
}
...
/**
*Returns all PV's in system
**/
func (ctrl *ProvisionController) getPVList() []*v1.PersistentVolume {
	var pvlist []*v1.PersistentVolume
	for _, pvo := range ctrl.volumes.List() {
		pvobj, ok := pvo.(*v1.PersistentVolume)
		if ok {
			pvlist = append(pvlist, pvobj)
		}
	}
	return pvlist
}
func (ctrl *ProvisionController) getConfiguration() map[string]string {

	configObj, found, err := ctrl.configuration.GetByKey(configMapName)
	if err != nil {
		glog.Errorf(" Error getting Config map for Provisioner Configuration")
	}
	if !found {
		glog.Errorf("Config map for Provisioner Configuration ( %q )not found", configMapName)
		//failled to find config map in which provisoner related configuation is maintained
	}

	switch cm := configObj.(type) {
	case *v1.ConfigMap:
		return cm.Data

	}
	glog.Errorf("No valid config map for configuration: %+v", configObj)
	return nil
}
