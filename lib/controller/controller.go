...
func (ctrl *ProvisionController) nodeAdded(obj interface{}) { 
	...
	glog.V(4).Info("Node added with addresses ", node.Status.Addresses)
	...
	for pname, provisioner := range ctrl.proviserners {
		if strings.Contains(pname, "/") { //so that default provisioner will get call only once
			err := provisioner.UpdateExport(ctrl.getPVList(), ctrl.getNodeList())
			if err != nil {
...
func (ctrl *ProvisionController) nodeDeleted(obj interface{}) { 
	...
	glog.V(4).Info("Node deleted with details ", node.Status.Addresses)
	//call all provisioner to update export
	for pname, provisioner := range ctrl.proviserners {
		if strings.Contains(pname, "/") { //so that default provisioner will get call only once
			err := provisioner.UpdateExport(ctrl.getPVList(), ctrl.getNodeList())
...
func (ctrl *ProvisionController) nodeUpdated(oldObj, newObj interface{}) {
	...
	glog.V(4).Info("Node addresses are updated  ", newnode.Status.Addresses)
	//call all provisioner to update export
	for pname, provisioner := range ctrl.proviserners {
		if strings.Contains(pname, "/") { //so that default provisioner will get call only once
			err := provisioner.UpdateExport(ctrl.getPVList(), ctrl.getNodeList())
